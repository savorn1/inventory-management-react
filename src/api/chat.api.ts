...
export const conversationsApi = {
  getAll: (page = 1, size = 20) =>
    http.get<PageResponse<ConversationDTO>>(
  ...
  getMembers: (id: number) =>
    http.get<ApiResponse<ConversationMemberDTO[]>>(
      `api/chat/conversations/${id}/members`,
    ),

  markAsRead: (id: number, messageId: number) =>
    http.patch<ApiResponse<unknown>>(
      `api/chat/conversations/${id}/read?messageId=${messageId}`,
      {},
    ),
};
