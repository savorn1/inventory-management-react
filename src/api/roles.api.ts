...
export const rolesApi = {
  ...
  getById: (id: number) => http.get<ApiResponse<RoleDTO>>(`api/roles/${id}`),
  ...
  delete: (id: number) => http.delete<ApiResponse<string>>(`api/roles/${id}`),
};

export const permissionsApi = {
  getAll: (page = 1, size = 200) =>
    http.get<PageResponse<PermissionDTO>>(
      `api/permissions?page=${page}&size=${size}`,
    ),
};
