export default {
  nav: {
    dashboard: "Dashboard",
    chat: "Chat",
    categories: "Categories",
    products: "Products",
    brands: "Brands",
    suppliers: "Suppliers",
    clients: "Clients",
    orders: "Orders",
    purchaseOrders: "Purchase Orders",
    payments: "Payments",
    stockMovements: "Stock Movements",
    openItems: "Open Items",
    users: "Users",
    roles: "Roles",
    pricing: "Pricing",
  },
  common: {
    add: "Add",
    edit: "Edit",
    delete: "Delete",
    cancel: "Cancel",
    save: "Save",
    update: "Update",
    saving: "Saving...",
    actions: "Actions",
    search: "Search",
    loading: "Loading...",
    name: "Name",
    description: "Description",
    status: "Status",
    noData: "No data found.",
    allStatus: "All Status",
    yes: "Yes",
    no: "No",
    optional: "Optional",
    none: "— None —",
    select: "— Select —",
    deleteConfirm: "Are you sure you want to delete this item?",
  },
  validation: {
    required: "This field is required.",
    minLength: "Must be at least {{min}} characters.",
    emailInvalid: "Enter a valid email address.",
    phoneInvalid: "Enter a valid phone number.",
    priceMin: "Price must be 0 or greater.",
    codeFormat: "Code must be uppercase letters, numbers, or underscores.",
    passwordRequired: "Password is required.",
    passwordMin: "Password must be at least {{min}} characters.",
  },
  auth: {
    signIn: "Sign in",
    subtitle: "Enter your credentials to continue",
    username: "Username",
    password: "Password",
    login: "Login",
    logout: "Logout",
    loggingIn: "Logging in...",
  },
  dashboard: {
    welcome: "Welcome to InvenAdmin",
    welcomeDesc:
      "Manage your inventory categories, products, brands, suppliers, and clients from one place. Use the sidebar to navigate between sections.",
    totalRevenue: "Total Revenue",
    orderStatus: "Order Status",
    productsByBrand: "Products by Brand",
    productsByCategory: "Products by Category",
    paymentChart: "Monthly Payment Overview",
    products: "Products",
    amount: "Amount",
    lastUpdated: "Last updated",
    refresh: "Refresh",
  },
  status: {
    active: "Active",
    inactive: "Inactive",
  },
  category: {
    title: "Categories",
    addTitle: "Add Category",
    searchPlaceholder: "Search categories...",
    noData: "No categories found.",
    name: "Name",
    namePlaceholder: "Category name",
    description: "Description",
    descriptionPlaceholder: "Optional description",
  },
  product: {
    title: "Products",
    addTitle: "Add Product",
    editTitle: "Edit Product",
    searchPlaceholder: "Search products...",
    noData: "No products found.",
    name: "Name",
    namePlaceholder: "Product name",
    description: "Description",
    descriptionPlaceholder: "Optional description",
    category: "Category",
    brand: "Brand",
    price: "Price ($)",
    pricePlaceholder: "0.00",
    image: "Image",
    images: "Images",
    uploadImage: "Click to upload image",
    changeImage: "Click to change image",
    addImages: "Add images",
  },
  brand: {
    title: "Brands",
    addTitle: "Add Brand",
    searchPlaceholder: "Search brands...",
    noData: "No brands found.",
    name: "Name",
    namePlaceholder: "Brand name",
  },
  supplier: {
    title: "Suppliers",
    addTitle: "Add Supplier",
    editTitle: "Edit Supplier",
    searchPlaceholder: "Search suppliers...",
    noData: "No suppliers found.",
    name: "Name",
    namePlaceholder: "Supplier name",
    email: "Email",
    emailPlaceholder: "supplier@email.com",
    phone: "Phone",
    phonePlaceholder: "+1 555 000 0000",
    address: "Address",
    addressPlaceholder: "Street, City, Country",
  },
  client: {
    title: "Clients",
    addTitle: "Add Client",
    editTitle: "Edit Client",
    searchPlaceholder: "Search clients...",
    noData: "No clients found.",
    name: "Name",
    namePlaceholder: "Full name",
    email: "Email",
    emailPlaceholder: "client@email.com",
    phone: "Phone",
    phonePlaceholder: "+1 555 000 0000",
    gender: "Gender",
    genderMale: "Male",
    genderFemale: "Female",
    genderOther: "Other",
    address: "Address",
    addressPlaceholder: "Street, City, Country",
  },
  user: {
    title: "Users",
    addTitle: "Add User",
    editTitle: "Edit User",
    searchPlaceholder: "Search users...",
    noData: "No users found.",
    username: "Username",
    usernamePlaceholder: "Username",
    password: "Password",
    passwordPlaceholder: "Password",
    passwordKeep: "(leave blank to keep current)",
    status: "Status",
    roles: "Roles",
    noRoles: "No roles available",
  },
  order: {
    title: "Orders",
    addTitle: "Create Order",
    searchPlaceholder: "Search orders...",
    searchByProduct: "Filter by product...",
    noData: "No orders found.",
    orderNo: "Order No.",
    client: "Client",
    clientRequired: "Please select a client.",
    orderDate: "Order Date",
    status: "Status",
    paymentStatus: "Payment",
    total: "Total",
    subtotal: "Subtotal",
    discount: "Discount",
    tax: "Tax",
    remark: "Remark",
    remarkPlaceholder: "Optional note...",
    orderInfo: "Order Information",
    details: "Order Items",
    addLine: "Add Item",
    noLines: 'No items added yet. Click "Add Item" to start.',
    product: "Product",
    productRequired: "Please select a product for each line.",
    searchProductPlaceholder: "Search product…",
    lineNotePlaceholder: "Note for this item…",
    detailsRequired: "Please add at least one item.",
    qty: "Qty",
    price: "Unit Price",
    lineDiscount: "Discount",
    lineTotal: "Total",
    createOrder: "Create Order",
    editTitle: "Edit Order",
    updateOrder: "Update Order",
    updateSuccess: "Order updated successfully.",
    updateFailed: "Failed to update order. Please try again.",
    updateStatus: "Update Status",
    statusPending: "Pending",
    statusConfirmed: "Confirmed",
    statusShipped: "Shipped",
    statusDelivered: "Delivered",
    statusCancelled: "Cancelled",
    statusProcessing: "Processing",
    statusCompleted: "Completed",
    statusUpdated: "Order status updated successfully.",
    statusUpdateFailed: "Failed to update order status.",
    allOrderStatus: "All Order Status",
    allPaymentStatus: "All Payment Status",
    paymentPaid: "Paid",
    paymentPartial: "Partial",
    paymentUnpaid: "Unpaid",
    paymentRefunded: "Refunded",
  },
  po: {
    title: "Purchase Orders",
    addTitle: "Create Purchase Order",
    searchPlaceholder: "Search by PO no. or supplier...",
    noData: "No purchase orders found.",
    allStatus: "All Status",
    poNo: "PO No.",
    supplier: "Supplier",
    supplierRequired: "Please select a supplier.",
    orderDate: "Order Date",
    expectedDate: "Expected Date",
    status: "Status",
    statusDraft: "Draft",
    statusOrdered: "Ordered",
    statusPartiallyReceived: "Partially Received",
    statusReceived: "Received",
    statusCancelled: "Cancelled",
    total: "Total",
    subtotal: "Subtotal",
    discount: "Discount",
    tax: "Tax",
    remark: "Remark",
    remarkPlaceholder: "Optional note...",
    poInfo: "Purchase Order Info",
    details: "Order Items",
    noLines: "No items added yet. Search a product above to add.",
    product: "Product",
    qty: "Qty",
    price: "Unit Price",
    lineDiscount: "Discount",
    lineTotal: "Total",
    ordered: "Ordered",
    received: "Received",
    receiveNow: "Receive Now",
    searchProductPlaceholder: "Search product…",
    detailsRequired: "Please add at least one item.",
    createPO: "Create Purchase Order",
    createSuccess: "Purchase order created successfully.",
    createFailed: "Failed to create purchase order. Please try again.",
    deleteSuccess: "Purchase order deleted.",
    deleteFailed: "Failed to delete purchase order.",
    updateStatus: "Update Status",
    statusUpdated: "Status updated successfully.",
    statusUpdateFailed: "Failed to update status.",
    receive: "Receive",
    receiveTitle: "Receive Items",
    receiveHint: "Enter the quantity received for each item.",
    receiveNoItems: "Enter at least one received quantity.",
    receiveSuccess: "Items received successfully.",
    receiveFailed: "Failed to record received items.",
    confirmReceive: "Confirm Receipt",
  },
  payment: {
    title: "Payments",
    noData: "No payments found.",
    paymentNo: "Payment No.",
    amount: "Amount",
    method: "Method",
    paidAt: "Paid At",
    updatePayment: "Update Status",
    liveConnected: "Live",
    liveDisconnected: "Disconnected",
    statusUpdated: "Payment status updated successfully.",
    statusUpdateFailed: "Failed to update payment status.",
  },
  chat: {
    title: "Chat",
    newConversation: "New Conversation",
    noConversations: "No conversations yet.",
    noMessages: "No messages yet. Say hello!",
    selectConversation: "Select a conversation to start chatting.",
    messagePlaceholder: "Type a message...",
    loadMore: "Load earlier messages",
    you: "You",
    typing: "Typing...",
    live: "Live",
    disconnected: "Disconnected",
    direct: "Direct",
    group: "Group",
    groupNamePlaceholder: "Group name",
    groupNameRequired: "Group name is required",
    start: "Start",
    messageDeleted: "Message deleted",
    reply: "Reply",
    replyingTo: "Replying to",
    members: "Members",
    mute: "Mute notifications",
    unmute: "Unmute notifications",
    setReminder: "Set a reminder",
    reminderIn20m: "In 20 minutes",
    reminderIn1h: "In 1 hour",
    reminderIn3h: "In 3 hours",
    reminderTomorrow: "Tomorrow at 9 am",
    reminderCustom: "Custom time",
    reminderSet: "Set reminder",
    sendHint: "Enter to send · Shift+Enter for new line",
  },
  stockMovement: {
    title: "Stock Movements",
    searchPlaceholder: "Search by product...",
    noData: "No stock movements found.",
    allTypes: "All Types",
    typeIn: "IN",
    typeOut: "OUT",
    product: "Product",
    type: "Type",
    qty: "Qty",
    reference: "Reference",
    remark: "Remark",
    createdBy: "By",
    date: "Date",
  },
  openItem: {
    title: "Open Items",
    addTitle: "Create Open Item",
    searchPlaceholder: "Search by item no...",
    noData: "No open items found.",
    allStatus: "All Status",
    itemNo: "Item No.",
    itemDate: "Item Date",
    status: "Status",
    statusDraft: "Draft",
    statusConfirmed: "Confirmed",
    statusCancelled: "Cancelled",
    totalQty: "Total Qty",
    remark: "Remark",
    remarkPlaceholder: "Optional note...",
    itemInfo: "Item Information",
    details: "Items",
    noLines: "No products added yet. Search a product above to add.",
    product: "Product",
    qty: "Qty",
    total: "Total",
    searchProductPlaceholder: "Search product…",
    detailsRequired: "Please add at least one item.",
    createItem: "Create Open Item",
    createSuccess: "Open item created successfully.",
    createFailed: "Failed to create open item. Please try again.",
    deleteSuccess: "Open item deleted.",
    deleteFailed: "Failed to delete open item.",
    confirm: "Confirm",
    confirmSuccess: "Open item confirmed successfully.",
    confirmFailed: "Failed to confirm open item.",
    cancel: "Cancel",
    cancelConfirm: "Are you sure you want to cancel this open item?",
    cancelSuccess: "Open item cancelled.",
    cancelFailed: "Failed to cancel open item.",
    detailTitle: "Item Details",
  },
  role: {
    title: "Roles",
    addTitle: "Add Role",
    editTitle: "Edit Role",
    searchPlaceholder: "Search roles...",
    noData: "No roles found.",
    name: "Name",
    namePlaceholder: "e.g. Administrator",
    code: "Code",
    codePlaceholder: "e.g. ADMIN",
    description: "Description",
    descriptionPlaceholder: "Optional description",
    isDefault: "Set as default role",
    permissions: "Permissions",
    permissionsSelected: "{{n}} permission(s) selected",
  },
};
