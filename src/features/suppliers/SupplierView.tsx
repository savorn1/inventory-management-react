...
export function SupplierView() {
  ...
  const { showModal, editingId, form, errors, saving, openAdd, openEdit, closeModal, setField, save, remove } =
    useCrud<SupplierDTO, Omit<SupplierDTO, 'id'>>({
      ...
      toForm: item => ({ name: item.name, phone: item.phone, email: item.email, address: item.address }),
      label: 'supplier',
      validate: f => {
    ...
    <div className="p-4 md:p-7 flex flex-col gap-5">
      ...
      <AppTable
        head={<>
          ...
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('supplier.name')}</th>
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('supplier.email')}</th>
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('supplier.phone')}</th>
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('supplier.address')}</th>
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('common.actions')}</th>
        </>}
        body={
          store.loading ? (
            <tr><td colSpan={6} className="text-center text-slate-400 py-12">{t('common.loading')}</td></tr>
          ) : store.items.length === 0 ? (
            <tr><td colSpan={6} className="text-center text-slate-400 py-12">{t('supplier.noData')}</td></tr>
            ...
            <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
              ...
              <td className="px-4 py-3">
                <div className="flex gap-2">
                  {auth.can('SUPPLIER_UPDATE') && <AppButton variant="edit" size="sm" onClick={() => openEdit(item)}>{t('common.edit')}</AppButton>}
                  {auth.can('SUPPLIER_DELETE') && <AppButton variant="delete" size="sm" onClick={() => remove(item.id)}>{t('common.delete')}</AppButton>}
                </div>
              </td>
            </tr>
          ))
      ...
      <AppModal show={showModal} title={editingId ? t('supplier.editTitle') : t('supplier.addTitle')} onClose={closeModal}>
        <form className="flex flex-col gap-4" onSubmit={e => { e.preventDefault(); save() }}>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-600">{t('supplier.name')} *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-600">{t('supplier.email')}</label>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-600">{t('supplier.phone')}</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-600">{t('supplier.address')}</label>
