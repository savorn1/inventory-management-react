    ...
    <div className="p-4 md:p-7 flex flex-col gap-5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {auth.can('PRODUCT_CREATE') && (
          <AppButton onClick={handleOpenAdd} className="ml-auto">+ {t('product.addTitle')}</AppButton>
        )}
      </div>
          ...
          store.loading ? (
            ...
            <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
              ...
              <td className="px-4 py-3">
                {item.imageUrl ? (
                  <img src={item.imageUrl} alt={item.name} className="w-10 h-10 object-cover rounded-md border border-slate-200" />
                ) : (
              ...
              <td className="px-4 py-3">
                <div className="flex gap-2">
                  {auth.can('PRODUCT_UPDATE') && <AppButton variant="edit" size="sm" onClick={() => handleOpenEdit(item)}>{t('common.edit')}</AppButton>}
      ...
      <AppModal show={showModal} title={editingId ? t('product.editTitle') : t('product.addTitle')} onClose={handleCloseModal}>
        <form className="flex flex-col gap-4" onSubmit={e => { e.preventDefault(); save() }}>
          {/* Image upload */}
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-600">{t('product.image')}</label>
              ...
              {imagePreview ? (
                <div className="relative w-full h-40 rounded-lg overflow-hidden border border-slate-200 group-hover:border-indigo-400 transition-colors">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <span className="text-white text-xs font-medium">{t('product.changeImage')}</span>
                  </div>
                </div>
              ) : (
                <div className="w-full h-40 rounded-lg border-2 border-dashed border-slate-200 group-hover:border-indigo-400 transition-colors flex flex-col items-center justify-center gap-2 text-slate-400 group-hover:text-indigo-400">
                  ...
                  <span className="text-xs font-medium">{t('product.uploadImage')}</span>
                </div>
              )}
            </label>
          </div>
          ...
          <div className="flex justify-end gap-2 mt-1">
            <AppButton variant="cancel" type="button" onClick={handleCloseModal}>{t('common.cancel')}</AppButton>
