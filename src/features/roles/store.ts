...
import type { RoleDTO, CreateRoleDTO, PermissionDTO } from '@/api/roles.api'
...
interface RoleStore {
  ...
  add: (payload: CreateRoleDTO) => Promise<void>
  update: (id: number, payload: CreateRoleDTO) => Promise<void>
  remove: (id: number) => Promise<void>
}

export const useRoleStore = create<RoleStore>((set, get) => ({
  items: [], allPermissions: [], loading: false, page: 1, size: 10,
  ...
  async fetchAll(p = get().page, s = get().size, q = get().search) {
    ...
    try {
      const res = await rolesApi.getAll(p, s, q)
