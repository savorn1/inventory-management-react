...
export function RoleFormModal({ show, editingId, saving, form, errors, allPermissions, onClose, onSave, onChange, onClearError }: Props) {
  ...
  function toggleModule(module: string) {
    ...
    if (isModuleAllChecked(module)) {
      onChange({ ...form, permissionIds: form.permissionIds.filter(id => !perms.some(p => p.id === id)) })
    } else {
  ...
  return (
    <AppModal show={show} title={editingId ? t('role.editTitle') : t('role.addTitle')} onClose={onClose}>
      <form className="flex flex-col gap-4" onSubmit={e => { e.preventDefault(); onSave() }}>
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <input value={form.name} onChange={e => { onChange({ ...form, name: e.target.value }); onClearError('name') }}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input value={form.code} onChange={e => { onChange({ ...form, code: e.target.value }); onClearError('code') }}
        ...
        <div className="flex justify-end gap-2 mt-1">
          <AppButton variant="cancel" type="button" onClick={onClose}>{t('common.cancel')}</AppButton>
