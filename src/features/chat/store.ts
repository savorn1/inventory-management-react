...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  async fetchConversations() {
    ...
    try {
      ...
      const privateMissing = res.data.filter(
        (c) => c.type === "PRIVATE" && !c.name && !get().convMembers[c.id],
      );
      privateMissing.forEach((c) => {
        conversationsApi
          ...
          .then((memberRes) => {
            ...
            set((state) => ({
              convMembers: { ...state.convMembers, [c.id]: memberIds },
            }));
            get()
      ...
      if (!mutedIds.includes(msg.conversationId)) {
        set((state) => ({
          unreadCounts: {
            ...state.unreadCounts,
            [msg.conversationId]:
              (state.unreadCounts[msg.conversationId] ?? 0) + 1,
          },
        }));
      }
  ...
  addReminder(reminder) {
    const next = [
      ...get().reminders,
      { ...reminder, id: `${Date.now()}-${reminder.messageId}` },
    ];
    saveReminders(next);
    set({ reminders: next });
  },
