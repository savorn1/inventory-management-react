...
export function MessageBubble({
  ...
  return (
    ...
    >
      {!isMe && senderName && (
        <span className="text-[11px] text-slate-400 ml-1 mb-0.5">
          {senderName}
        </span>
      )}
      <div className={`flex ${isMe ? "justify-end" : "justify-start"} w-full`}>
        ...
        {editing ? (
          ...
          >
            {replyToMsg && (
              <div
                className={`text-xs px-2 py-1.5 mb-1.5 rounded-lg border-l-2 ${
                  isMe
              ...
              >
                <div
                  className={`font-medium mb-0.5 truncate ${isMe ? "text-indigo-200" : "text-slate-600"}`}
                >
                  {replyToSenderName ?? "…"}
                </div>
                <div
                  className={`truncate ${isMe ? "text-indigo-200" : "text-slate-500"}`}
                >
                  {replyToMsg.deleted ? (
            ...
            <div
              className={`text-[10px] mt-0.5 ${isMe ? "text-indigo-200 text-right" : "text-slate-400"}`}
            >
