...
interface UserStore {
  ...
  search: string
  statusFilter: string
  fetchAll: (p?: number, s?: number, name?: string, status?: string) => Promise<void>
  ...
  update: (id: number, payload: Omit<CreateSysUserDTO, 'password'> & { password?: string; roleIds?: number[] }) => Promise<void>
  remove: (id: number) => Promise<void>
}

export const useUserStore = create<UserStore>((set, get) => ({
  items: [], loading: false, page: 1, size: 10,
  totalCount: 0, totalPages: 0, hasNext: false, hasPrev: false,
  search: '', statusFilter: '',

  async fetchAll(p = get().page, s = get().size, name = get().search, status = get().statusFilter) {
  ...
  searchByName(q) { set({ search: q }); return get().fetchAll(1, get().size, q, get().statusFilter) },
  filterByStatus(status) { set({ statusFilter: status }); return get().fetchAll(1, get().size, get().search, status) },
