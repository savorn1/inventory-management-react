...
export function UserView() {
  ...
  async function save() {
    ...
    if (!editingId && !form.password)
      ...
      e.password = t("validation.passwordMin", { min: 6 });
    ...
    try {
      if (editingId !== null) {
        const payload: {
          ...
          status: string;
          roleIds: number[];
        } = { name: form.name, status: form.status, roleIds: form.roleIds };
        if (form.password) payload.password = form.password;
        await store.update(editingId, payload);
      } else {
        const newUser = await store.add({
          name: form.name,
          password: form.password,
          status: form.status,
        });
        if (form.roleIds.length && newUser?.id) {
          await store.update(newUser.id, {
            name: newUser.name,
          ...
          store.loading ? (
            ...
            store.items.map((item, idx) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${item.status === "ACTIVE" ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                  >
                    {item.status === "ACTIVE"
                      ? t("status.active")
                      : t("status.inactive")}
