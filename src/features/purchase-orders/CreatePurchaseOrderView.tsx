...
export function CreatePurchaseOrderView() {
  ...
  useEffect(() => {
    suppliersApi.getAll(1, 500).then((res) => setSuppliers(res.data));
  }, []);

  // auto-focus qty cell of newly added / updated row
  useEffect(() => {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!form.supplierId) return alert(t("po.supplierRequired"));
    if (lines.length === 0) return alert(t("po.detailsRequired"));
    ...
    try {
      await store.add({
        ...
        details: lines.map((l) => ({
          ...
          discount:
            l.discountType === "pct"
              ? (l.qty * l.price * l.discount) / 100
              : l.discount,
        })),
      });
      toast.add(t("po.createSuccess"));
      navigate("/purchase-orders");
    } catch {
      toast.add(t("po.createFailed"), "error");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="p-4 md:p-7 max-w-5xl mx-auto flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-slate-800">{t("po.addTitle")}</h1>
      </div>
      ...
      <form className="flex flex-col gap-5" onSubmit={submit}>
        ...
        <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-5 flex flex-col gap-4">
          ...
          <AppProductCombobox
            ...
            placeholder={t("po.searchProductPlaceholder")}
            allowOutOfStock
          />
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              onClick={() =>
                                updateLine(idx, {
                                  discountType:
                                    line.discountType === "flat" ? "pct" : "flat",
                                  discount: 0,
                                })
                              }
        ...
        <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-5">
          <div className="flex flex-col gap-3 max-w-xs ml-auto">
            ...
            <div className="flex items-center justify-between text-sm text-slate-600">
              <label className="shrink-0">{t("po.discount")}</label>
              <div className="flex items-center gap-1">
            ...
            <div className="flex items-center justify-between text-sm text-slate-600">
              <label className="shrink-0">{t("po.tax")}</label>
              <div className="flex items-center gap-1">
                <span className="text-slate-400">$</span>
                <input
                  type="number"
        ...
        <div className="flex justify-end gap-3">
          ...
          <AppButton type="submit" disabled={saving}>
            {saving ? t("common.saving") : t("po.createPO")}
          </AppButton>
        </div>
      </form>
    </div>
