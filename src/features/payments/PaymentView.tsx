...
export function PaymentView() {
  ...
  async function saveStatus() {
    ...
    try {
      ...
      toast.add(err instanceof Error ? err.message : t('payment.statusUpdateFailed'), 'error')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="p-4 md:p-7 flex flex-col gap-5">
      ...
      <AppTable
        head={<>
          ...
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('payment.paymentNo')}</th>
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('order.orderNo')}</th>
          <th className="px-4 py-3 text-right text-xs font-bold text-slate-500 uppercase tracking-wide">{t('payment.amount')}</th>
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('payment.method')}</th>
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('order.paymentStatus')}</th>
          <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">{t('payment.paidAt')}</th>
        ...
        body={
          store.loading ? (
            ...
            <tr><td colSpan={8} className="text-center text-slate-400 py-12">{t('payment.noData')}</td></tr>
          ) : store.items.map((item, idx) => (
            <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
              <td className="px-4 py-3 text-slate-400">{(store.page - 1) * store.size + idx + 1}</td>
              <td className="px-4 py-3 font-mono text-sm font-semibold text-indigo-700">{item.paymentNo}</td>
              ...
              {auth.can('ORDER_UPDATE') && (
                <td className="px-4 py-3">
                  <AppButton
                    variant="edit"
                    size="sm"
                    onClick={() => setStatusModal({ paymentId: item.id, paymentNo: item.paymentNo, status: item.status })}
                  >
      ...
      >
        <div className="flex flex-col gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('order.paymentStatus')}</label>
            <select
