...
interface OrderActions {
  fetchAll:             (page?: number, size?: number, search?: string, statusFilter?: string, paymentStatusFilter?: string) => Promise<void>
...
export const useOrderStore = create<OrderStore>((set, get) => ({
  ...
  async fetchAll(
    ...
    try {
      const res = await ordersApi.getAll(page, size, search, statusFilter, paymentStatusFilter)
      set({
        items:      res.data,
  ...
  setStatusFilter(status) {
    ...
    return get().fetchAll(1, get().size, get().search, status, get().paymentStatusFilter)
  },

  setPaymentFilter(paymentStatus) {
    set({ paymentStatusFilter: paymentStatus })
    return get().fetchAll(1, get().size, get().search, get().statusFilter, paymentStatus)
  },
