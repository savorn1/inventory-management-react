...
export function CreateOrderView() {
  ...
  useEffect(() => {
    Promise.all([clientsApi.getAll(1, 500), productsApi.getAll(1, 500)]).then(([c, p]) => {
      setClients(c.data)
      setProducts(p.data)
    })
  }, [])
  ...
  const total    = useMemo(() => subtotal - Number(form.discount) + Number(form.tax), [subtotal, form.discount, form.tax])

  function addLine() {
    setLines(prev => [...prev, { productId: 0, productName: '', qty: 1, price: 0, discount: 0, total: 0 }])
  }
  ...
  function updateLine(idx: number, updates: Partial<LineItem>) {
    setLines(prev => {
      ...
      const l = next[idx]
      next[idx] = { ...l, total: Math.max(0, l.qty * l.price - Number(l.discount)) }
      return next
    })
  }
  ...
  function onProductChange(idx: number, productId: number) {
    ...
    if (prod) {
      updateLine(idx, { productId: prod.id, productName: prod.name, price: prod.price })
    }
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (lines.some(l => !l.productId)) return alert(t('order.productRequired'))
  ...
  return (
    <div className="p-4 md:p-7 max-w-5xl mx-auto flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-slate-800">{t('order.addTitle')}</h1>
      </div>

      <form className="flex flex-col gap-5" onSubmit={submit}>
        {/* Order Info */}
        <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-5 flex flex-col gap-4">
          <h2 className="text-sm font-bold text-slate-600 uppercase tracking-wide">{t('order.orderInfo')}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-600">{t('order.client')} *</label>
              <select value={form.clientId} onChange={e => setForm(f => ({ ...f, clientId: Number(e.target.value) }))} required
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-600">{t('order.orderDate')}</label>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-600">{t('order.status')}</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-600">{t('order.remark')}</label>
        ...
        <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-5 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-slate-600 uppercase tracking-wide">{t('order.details')}</h2>
            <AppButton type="button" size="sm" onClick={addLine}>+ {t('order.addLine')}</AppButton>
          </div>

          {lines.length === 0 ? (
            <div className="text-center text-slate-400 py-8 text-sm">{t('order.noLines')}</div>
          ) : (
            <div className="overflow-x-auto -mx-1">
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="px-2 py-2 text-left text-xs font-bold text-slate-500 uppercase tracking-wide min-w-[180px]">{t('order.product')}</th>
                    <th className="px-2 py-2 text-left text-xs font-bold text-slate-500 uppercase tracking-wide w-20">{t('order.qty')}</th>
                    <th className="px-2 py-2 text-left text-xs font-bold text-slate-500 uppercase tracking-wide w-28">{t('order.price')}</th>
                    <th className="px-2 py-2 text-left text-xs font-bold text-slate-500 uppercase tracking-wide w-28">{t('order.lineDiscount')}</th>
                    <th className="px-2 py-2 text-right text-xs font-bold text-slate-500 uppercase tracking-wide w-28">{t('order.lineTotal')}</th>
                  ...
                  {lines.map((line, idx) => (
                    <tr key={idx} className="border-b border-slate-50">
                      <td className="px-2 py-2">
                        <select value={line.productId} onChange={e => onProductChange(idx, Number(e.target.value))}
                      ...
                      <td className="px-2 py-2 text-right font-semibold text-slate-700">${fmt(line.total)}</td>
                      <td className="px-2 py-2">
        ...
        <div className="flex justify-end gap-3">
          <AppButton type="button" variant="cancel" onClick={() => navigate('/orders')}>{t('common.cancel')}</AppButton>
          <AppButton type="submit" disabled={saving}>{saving ? t('common.saving') : t('order.createOrder')}</AppButton>
        </div>
      </form>
    </div>
  )
