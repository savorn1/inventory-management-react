...
export function EditOrderView() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      clientsApi.getAll(1, 500),
      ordersApi.getById(orderId),
    ])
      .then(([clientsRes, orderRes]) => {
      ...
      .catch(() => {
        toast.add("Failed to load order.", "error");
        navigate("/orders");
      })
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.add(t("order.updateSuccess"));
      navigate("/orders");
    } catch {
      toast.add(t("order.updateFailed"), "error");
    } finally {
      setSaving(false);
    }
  }
  ...
  if (loading) {
    return (
      <div className="p-4 md:p-7 flex items-center justify-center text-slate-400 text-sm">
        {t("common.loading")}
      </div>
    );
  }
    ...
    <div className="p-4 md:p-7 max-w-5xl mx-auto flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {orderNo && (
            <p className="text-sm text-slate-500 font-mono">{orderNo}</p>
          )}
        </div>
      </div>
      ...
      <form className="flex flex-col gap-5" onSubmit={submit}>
        ...
        <div className="flex justify-end gap-3">
          ...
          <AppButton type="submit" disabled={saving}>
            {saving ? t("common.saving") : t("order.updateOrder")}
          </AppButton>
        </div>
      </form>
    </div>
